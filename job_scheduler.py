...
from bst_demo import BSTDemo


def get_job_input_details():
    start_timee = input("Enter the time in hh:mm format, example 18:30 or 6:30-> ")
    while True:
        try:
            datetime.strptime(start_timee, '%H:%M')
        except ValueError:
            print("Incorrect time format, should be hh:mm")
            start_timee = input("Enter the time in hh:mm format, ex 18:30 or 6:30-> ")
        else:
            break
    duration_of_jobb = input("Enter the duration of the job in minutes, ex 60-> ")
    while True:
        try:
            int(duration_of_jobb)
        except ValueError:
            print("Please enter a number for number of minutes")
            duration_of_jobb = input("Enter the duration of the job in minutes, ex 60-> ")
        else:
            break
    job_namee = input("Enter the name of the job (case sensitive)-> ")
    return start_timee, duration_of_jobb, job_namee
